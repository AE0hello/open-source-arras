...
const { bossBattle } = require("./gamemodes/bossbattle.js");
const { surviveArenaCloser } = require("./gamemodes/surviveArenaCloser.js");
...
class gamemodeManager {
  constructor() {
    ...
    this.gameTwoTDMOneDominator = new TwoTDMOneDominator(global.gameManager);
    this.gameBossBattle = new bossBattle(global.gameManager);
    this.gameSurviveArenaCloser = new surviveArenaCloser(global.gameManager);
  }

  request(type) {
    if (type == "start") {
      if (Config.special_boss_spawns) {
        this.gameSiege.start(Config.maze_type ?? false);
      }
      if (Config.ASSAULT) {
        this.gameAssault.start();
      }
      if (Config.tag) {
        Config.tag_data.initAndStart();
      }
      if (Config.domination) {
        this.gameDomination.start();
      }
      if (Config.mothership) {
        this.gameMothership.start();
      }
      ...
      if (Config.twoTDMOneDominator) {
        this.gameTwoTDMOneDominator.start();
      }
      if (Config.bossbattle) {
        this.gameBossBattle.start();
      }
      if (Config.surviveArenaCloser) {
        this.gameSurviveArenaCloser.start();
      }
    }
    if (type == "loop") {
  ...
  terminate() {
    if (Config.special_boss_spawns) {
      this.gameSiege.reset();
    }
    if (Config.ASSAULT) {
      this.gameAssault.reset();
    }
    if (Config.tag) {
      Config.tag_data.resetAndStop();
    }
    if (Config.domination) {
      this.gameDomination.reset();
    }
    if (Config.mothership) {
      this.gameMothership.reset();
    }
    if (Config.clan_wars) {
      this.gameClanwars.reset();
    }
    if (Config.twoTDMOneDominator) {
      this.gameTwoTDMOneDominator.reset();
    }
    if (Config.bossbattle) {
      this.gameBossBattle.reset();
    }
    if (Config.surviveArenaCloser) {
      this.gameSurviveArenaCloser.reset();
    }
  }
  ...
  redefine(theshit) {
    ...
    this.gameSurviveArenaCloser.redefine(theshit);
  }
}
