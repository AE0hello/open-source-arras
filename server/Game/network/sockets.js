...
const bans = global.bans || (global.bans = []);
...
class socketManager {
  constructor(parent) {
    ...
    this.bans = [];
    // Import permissions
    for (const entry of require("../permissions.js")) {
  ...
  ban(socket, reason) {
    ...
    const s = this.clients.filter((c) => c.ip === socket.ip);
  ...
  chatLoop() {
    ...
    for (const view of global.gameManager.views) {
    ...
    if (index != -1) {
      ...
      if (player.body != null) {
        ...
        if (socket.status.transferred) {
          ...
          if (Config.clan_wars) {
            Config.clan_wars_ft.remove(player.body);
          }
          player.body.invuln = false;
          player.body.kill();
          player.body.destroy();
  ...
  incoming(message, socket) {
    ...
    switch (m.shift()) {
      ...
      case "s": { // spawn request
        ...
        const b = bans.find((ban) => ban.ip === socket.ip);
        if (b) {
        ...
        const name = m[0].replace(Config.BANNED_CHARACTERS_REGEX, "");
        const needsRoom = m[1];
        const autoLVLup = m[2];
        let transferbodyID = m[3];
        if (transferbodyID) {
          transferbodyID = transferbodyID.replace(name, "");
        }
        if (global.gameManager.arenaClosed) {
          if (needsRoom) {
            ...
            socket.talk("m", 5_000, "Arena Closed.");
          }
          return;
        };
        // Verify it
        ...
        if (needsRoom) {
          if (Config.hidden) {
            return socket.close();
          } // If the server is hidden then just kick the client.
      ...
      case "H": {
        ...
        for (const e of entities.values()) {
          if (e.isDominator || e.isMothership) {
            ent.push(e);
          }
        }
        body.emit("control", { body });
        if (body.underControl) {
          const relinquishedControlMessage =
        ...
        if (Config.mothership) {
          ...
          const dominators = ent.map((entry) => {
            if (entry.isDominator && entry.team === player.body.team && !entry.underControl) {
              return entry;
            }
          }).filter(x => x);
          ...
          const dominator = dominators.shift();
      ...
      case "T": {
        ...
        if (player.body && socket.status.lastTank != player.body.index) {
          ...
          const allRoots = [],
            rerootUpgradeTree = [];
          for (const i of player.body.index.split("-")) {
            const mockup = mockupData.find(o => o.index === `${i}`);
          ...
          for (const i of rerootUpgradeTree) {
            const ind = Class[i].index;
      ...
      case "DTA": {
        if (player.body && player.body.skill.level >= Config.tier_multiplier * Config.daily_tank.tier && Config.daily_tank.ads.enabled && !socket.status.daily_tank_watched_ad) {
          const chosenAd = ran.choose(Config.daily_tank.ads.source);
          const isImage = chosenAd.file.endsWith(".png") || chosenAd.file.endsWith(".jpg") || chosenAd.file.endsWith(".jpeg");
  ...
  spectateEntity(possible, socket) {
    ...
    for (const entry of entities.values()) {
      ...
      if (possible.includes("players") && entry.isPlayer) {
        entries.push(entry);
      }
      if (possible.includes("bots") && entry.isBot) {
        entries.push(entry);
      }
    }
    if (!entries.length) {
  ...
  update(gui) {
    ...
    // Update the upgrades
    const upgrades = [];
    const skippedUpgrades = [0];
    for (let i = 0; i < b.upgrades.length; i++) {
      const upgrade = b.upgrades[i];
  ...
  initalizePlayer(epackage, socket) {
    const name = epackage.name;
    const autoLVLup = epackage.autoLVLup;
    const transferbodyID = epackage.transferbodyID;
    const eastereggs = {
    ...
    if (transferbodyID) {
      const bodyInfo = global.travellingPlayers.find(i => i.id === transferbodyID);
      if (bodyInfo) {
        spawn = false;
        socket.player = socket.spawn(name);
        socket.player.body.upgrades = [];
        for (const def of bodyInfo.definition) {
          if (def in Class) {
  ...
  newPlayer(socket) {
    const { player, loc } = this.getSpawnLocation(socket.rememberedTeam);
  ...
  spawn = (socket, name) => {
    ...
    if (filter.length) {
      ...
      if (Class.menu_tanks) {
        const string = Class.menu_tanks.UPGRADES_TIER_0[0];
        if (string !== "basic") {
    ...
    if (!filter.length) {
      switch (Config.mode) {
        ...
        case "clan": {
          ...
          if (!body.clan) {
            const loop = setInterval(() => {
              for (const e of Config.clan_wars_ft.getClans()) {
        ...
        default: {
          const team = filter.length ? player.team : getRandomTeam();
          body.team = team;
  ...
  preparePlayer(socket, player, body, doNotTakeAction = {}) {
    ...
    if (!doNotTakeAction.dontSendWelcomeMessage) {
      const msg = Config.spawn_message.split("\n");
  ...
  generateMockup(index) {
    ...
    let mock;
    const find = Object.keys(Class).find(o => Class[o] && Class[o].index === index);
    if (find) {
  ...
  sendMockup(index, socket) {
    for (const splittedIndex of index.toString().split("-")) {
      if (socket.status.mockupData.receivedIndexes.includes(splittedIndex)) {
        continue;
      } // Do NOT continue if we have the mockup already.

      const index = parseInt(splittedIndex); // Parse it, without this wont work for some reason.
      // Now we need to find the mockup.
      let mockup = mockupData.find(o => o.index === `${index}`);
      if (!mockup) { // If not, then make one.
        const e = this.generateMockup(index);
      ...
      for (const turrets of mockup.turrets) {
      ...
      if (mockup.sendAllMockups) { // Send all of its upgrades if needed to prevent bugs.
        // Target the upgrades
        for (const upgrades of mockup.upgrades) {
          for (const i of upgrades.index.split("-")) { // Split the indexes.
  ...
  sendMockupUpgrades(index, socket) {
    for (const splittedIndex of index.toString().split("-")) {
      if (socket.status.mockupData.receivedUpgradePackIndexes.includes(splittedIndex)) {
        continue;
      } // Do NOT continue if we have the mockup already.
      this.sendMockup(index, socket);
      const parsedindex = parseInt(splittedIndex);
      let mockup = mockupData.find(o => o.index === `${parsedindex}`);
      if (!mockup) {
        const e = this.generateMockup(parsedindex);
        mockup = mockupData.find(o => o.index === `${e.index}`);
      }
      ...
      for (const { index } of mockup.upgrades) {
        for (const i of index.toString().split("-")) {
          this.sendMockupUpgrades(i, socket);
        }
      }
    }
  ...
  eyes(socket) {
    const check = (camera, obj) => {
      const fov = global.gameManager.arenaClosed ? 1.6 : 1;
    ...
    const o = {
      ...
      gazeUpon: (updateCam = false) => {
        ...
        const lastCycle = global.gameManager.room.lastCycle;
        // else update it.
        ...
        if (player.body != null) {
          ...
          if (player.body.isDead()) {
            const purge = () => player.body = null; // Remove our bonded body.
            ...
            const die = () => { // The only reason this exist is because of bacteria's abilities.
            ...
            if (player.body.master.label == "Bacteria") { // Why not trigger bacteria's abilities :)
              const exit = () => die();
              const newgui = (player) => this.newgui(player);
            ...
            const x = player.body.cameraOverrideX === null ? player.body.photo.x : player.body.cameraOverrideX,
        ...
        if (!Config.load_all_mockups) {
          for (const upgrade of (player.body?.upgrades || [])) {
  ...
  deltaHandler = (() => {
    const Delta = class {
      ...
      update(id = 0, ...args) {
        if (!this.data[id]) {
          this.data[id] = this.finder([]);
        }
        const old = this.data[id];
        const now = this.finder(args);
        this.data[id] = now;
        this.now = now;
        let oldIndex = 0;
        let nowIndex = 0;
    ...
    const makeLeaderboardList = (list, args) => {
      const topTen = [];
      for (let i = 0; i < 10 && list.length; i++) {
        let top,
          is = 0;
        for (let j = 0; j < list.length; j++) {
          const val = list[j].skill.score;
          if (val > is) {
            is = val;
            top = j;
          }
    ...
    const makeLeaderboardHPList = (list) => {
      const topTen = [];
      for (let i = 0; i < 10 && list.length; i++) {
        let top,
          is = 0;
    ...
    // Deltas
    const minimapAll = new Delta(5, args => {
      const all = [];
    ...
    const minimapTeams = new Delta(3, args => {
      const all = [];
      for (const my of entities.values()) {
    ...
    const minimapAllTeams = new Delta(3, args => {
      const all = [];
      for (const my of entities.values()) {
    ...
    const globalLeaderboard = new Delta(7, args => {
      const list = [];
      if (Config.tag) {
        const teams = Config.tag_data.getData();
      ...
      if (Config.mothership) {
        const teams = Config.mothership_data.getData();
        for (let i = 0; i < teams.length; i++) {
          const m = teams[i];
          if (!m.isDead()) {
            list.push({
    ...
    const defaultLeaderboard = new Delta(7, args => {
      const list = [];
      for (const instance of entities.values()) {
        if (instance.settings.leaderboardable &&
                    instance.settings.drawShape &&
    ...
    const playerLeaderboard = new Delta(7, args => {
      const list = [];
      for (const instance of entities.values()) {
        if (
          instance.isPlayer &&
    ...
    const bossLeaderboard = new Delta(7, args => {
      const list = [];
      for (const instance of entities.values()) {
        if (
          (instance.isBoss ||
    ...
    setInterval(() => {
      ...
      for (const socket of subscribers) {
        ...
        if (!socket.status.hasSpawned || socket.status.selectedLeaderboard == "stop") {
          continue;
        }
        const sl = socket.status.selectedLeaderboard;
        ...
        const team = socket.status.seesAllTeams ? minimapAllTeamsUpdate : minimapTeamUpdates;

        // Send the leaderboard tanks' mockups
        if (global.gameManager.gameHandler.active) {
          for (const e of getLeaderboard.now) {
  ...
  sendToServer(socket, server) {
    if (!socket.player?.body || socket.status.transferred) {
      return;
    }
  ...
  connect(socket, req) {
    ...
    const trafficMonitoring = setInterval(() => this.traffic(socket), 1500);
    ...
    // Send welcome message for Nostalgia gamemode
    if (global.gameManager && global.gameManager.name === "Nostalgia") {
      socket.talk("m", Config.popup_message_duration, "This is a reference to \"ARRAS.IO BOSSES GALORE!\" by Voizion https://www.youtube.com/watch?v=4prHdG_A9r4");
    }
  };

  disconnect(socket) {
    const check = this.clients.find(o => o.id === socket.id);
    if (check) {
