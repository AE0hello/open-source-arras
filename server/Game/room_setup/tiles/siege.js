...
const addTileToBossSpawnTile = (tile, room) => {
  if (!room.spawnable.bossSpawnTile) {
    room.spawnable.bossSpawnTile = [];
  }
  room.spawnable.bossSpawnTile.push(tile);
};
const bossTick = (tile, pushTo, allow) => {
  for (let i = 0; i < tile.entities.length; i++) {
    const entity = tile.entities[i];
    if (!entity.isBoss &&
            !entity.master.master.isBoss &&
...
tileClass.bossSpawn = new Tile({
  ...
  NAME: "Boss Spawn",
  INIT: (tile, room) => {
    if (!Config.blitz && !Config.fortress && !Config.citadel) {
      addTileToBossSpawnTile(tile, room);
    }
  },
...
tileClass.sbase1 = new Tile({
  ...
  NAME: "Sanctuary Tile",
  INIT: (tile, room) => {
    if (!room.spawnable[TEAM_BLUE]) {
      room.spawnable[TEAM_BLUE] = [];
    }
  ...
  TICK: (tile, room) => {
    ...
    for (let i = 0; i < tile.entities.length; i++) {
      ...
      if (entity.pushability && entity.isBot) {
        const dirToCenter = Math.atan2(room.height / pushy - entity.y - room.height / 2, room.width / pushx - entity.x - room.width / 2);
