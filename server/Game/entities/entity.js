...
class Entity extends EventEmitter {
  constructor(position, master) {
    ...
    this.addToGrid = () => {
      if (!this.collidingBond && this.bond != null) {
        return;
      }
      if (!this.isInGrid) {
        this.isInGrid = true;
  ...
  addController(newIO) {
    ...
    for (let oldId = 0; oldId < this.controllers.length; oldId++) {
      for (let newId = 0; newId < newIO.length; newId++) {
        const oldIO = this.controllers[oldId];
        const io = newIO[newId];
  ...
  giveUp(player, name = "Mothership") {
    ...
    const fakeBody = new Entity({ x: player.body.x, y: player.body.y });
  ...
  define(defs, emitEvent = true, overwriteindex = true, overrideDefs = true) {
    ...
    const set = ensureIsClass(defs[0]);
    this.props.clear();
    this.store = {};
    ...
    if (set.index != null && overwriteindex) {
    ...
    if (set.DISPLAY_NAME != null) {
      this.displayName = set.DISPLAY_NAME;
    }
    if (set.TYPE != null) {
      this.type = set.TYPE;
    }
    if (set.WALL_TYPE != null) {
      this.walltype = set.WALL_TYPE;
    }
    if (set.SHAPE != null) {
    ...
    if (set.IGNORED_BY_AI != null) {
      this.ignoredByAi = set.IGNORED_BY_AI;
    }
    ...
    if (set.NO_COLLISIONS) {
      this.settings.no_collisions = set.NO_COLLISIONS;
    }
    ...
    if (set.NECRO != null) {
      ...
      for (const shape of this.settings.necroTypes) {
      ...
      this.necro = (host) => {
        const gun = this.settings.necroDefineGuns[host.shape];
        if (!gun || !gun.checkShootPermission()) {
          return false;
        }

        const savedFacing = host.facing;
        const savedSize = host.SIZE;
    ...
    if (set.KILL_MESSAGE != null) {
      this.settings.killMessage = set.KILL_MESSAGE === "" ? "Killed" : set.KILL_MESSAGE;
    }
    ...
    if (set.DEFEAT_MESSAGE) {
      this.settings.defeatMessage = true;
    }
    if (set.HEALER) {
      this.healer = true;
    }
    ...
    if (set.ALPHA != null) {
      this.alpha = (typeof set.ALPHA === "number") ? set.ALPHA : set.ALPHA[1];
    ...
    if (set.BORDERLESS != null) {
      this.borderless = set.BORDERLESS;
    }
    if (set.DRAW_FILL != null) {
      this.drawFill = set.DRAW_FILL;
    }
    if (set.IS_IMMUNE_TO_TILES) {
      this.immuneToTiles = set.IS_IMMUNE_TO_TILES;
    }
    if (set.TEAM != null) {
      this.team = set.TEAM;
    ...
    if (set.RESET_UPGRADES || set.RESET_STATS) {
      const caps = this.skill.caps.map(x => x);
    ...
    if (set.RESET_UPGRADE_MENU) {
      this.upgrades = [];
    }
    if (set.ARENA_CLOSER != null) {
      this.isArenaCloser = set.ARENA_CLOSER, this.ac = set.ARENA_CLOSER;
    }
    if (set.BRANCH_LABEL != null) {
      this.branchLabel = set.BRANCH_LABEL;
    }
    if (set.BATCH_UPGRADES != null) {
      this.batchUpgrades = set.BATCH_UPGRADES;
    }
    for (let i = 0; i < Config.tier_cap; i++) {
      const tierProp = "UPGRADES_TIER_" + i;
      if (set[tierProp] != null && emitEvent) {
        for (let j = 0; j < set[tierProp].length; j++) {
          ...
          const redefineAll = upgrades.includes(true);
          const trueUpgrades = upgrades.slice(0, upgrades.length - redefineAll); // Ignore last element if it's true
          for (const k of trueUpgrades) {
            const e = ensureIsClass(k);
            index += e.index + "-";
          }
          this.upgrades.push({
            class: trueUpgrades,
    ...
    if (set.SKILL_CAP != null) {
      const skillCapsToSet = Array.isArray(set.SKILL_CAP) ? set.SKILL_CAP : SKILL_ORDER.map(name =>
    ...
    if (set.SKILL != null) {
      const skillsToSet = Array.isArray(set.SKILL) ? set.SKILL : SKILL_ORDER.map(name =>
    ...
    if (typeof set.defineLevelSkillPoints === "function") {
      this.skill.LSPF = set.defineLevelSkillPoints;
    }
    if (set.RECALC_SKILL != null) {
      const score = this.skill.score;
      this.skill.reset();
    ...
    if (set.BODY != null) {
      ...
      if (set.BODY.KNOCKBACK != null) {
        this.KNOCKBACK = set.BODY.KNOCKBACK;
      }
    ...
    if (set.RESET_EVENTS) {
      for (const { event, handler, once } of this.definitionEvents) {
    ...
    if (Array.isArray(this.rerootUpgradeTree)) {
      let finalRoot = "";
      for (const root of this.rerootUpgradeTree) {
        finalRoot += root + "\\/";
      }
      this.rerootUpgradeTree = finalRoot.substring(0, finalRoot.length - 2);
    }
    if (set.ON_MINIMAP != null) {
      this.allowedOnMinimap = set.ON_MINIMAP;
    }
    if (set.TURRETS != null) {
    ...
    if (set.ON != null) {
      for (const { event, handler, once = false } of set.ON) {
  ...
  refreshBodyAttributes() {
    ...
    if (this.settings.diesAtRange || !this.range) {
  ...
  bindToMaster(position, bond, isInvulnerable) {
    ...
    if (Array.isArray(position)) {
      position = { SIZE: position[0], X: position[1], Y: position[2], ANGLE: position[3], ARC: position[4], LAYER: position[5] };
    }
    position.SIZE ??= 10;
    position.X ??= 0;
    position.Y ??= 0;
    ...
    position.ARC ??= 360;
    position.LAYER ??= 0;
    const _off = new Vector(position.X, position.Y);
  ...
  isBeingViewed(addToNearby = true) {
    const boolean = checkIfInView(false, addToNearby, global.gameManager.clients, this);
    return boolean;
  }
  ...
  upgrade(number, branchId) {
    ...
    if (number.isDailyUpgrade) {
      ...
      if (!Config.daily_tank.ads.enabled) {
        hasWatchedAd = true;
      }
      const requestedIndex = parseInt(number.tank);
      if (requestedIndex === ensureIsClass(Config.daily_tank.tank).index && this.skill.level >= Config.tier_multiplier * Config.daily_tank.tier) {
        if (hasWatchedAd) {
          ...
          this.sendMessage("You must watch an ad before you can upgrade.");
        }
      }
    } else {
      ...
      if (number < this.upgrades.length && this.skill.level >= this.upgrades[number].level) {
        upgraded = true;
        const upgrade = this.upgrades[number], upgradeClass = upgrade.class, upgradeBranch = upgrade.branch, redefineAll = upgrade.redefineAll;
        if (redefineAll) {
  ...
  friction() {
    const motion = this.velocity.length,
      excess = motion - this.maxSpeed;
    if (excess > 0 && this.damp) {
      const k = this.damp / global.gameManager.roomSpeed,
        drag = excess / (k + 1),
    ...
    if (!this.settings.canGoOutsideRoom) {
      if (Config.arena_shape === "circle") {
        ...
        if (dist > global.gameManager.room.width - global.gameManager.room.width / 2) {
          const strength = (dist - global.gameManager.room.width / 2) * Config.room_bound_force / (Config.run_speed * 350);
  ...
  contemplationOfMortality() {
    ...
    if (this.damageReceived > 0) {
      ...
      for (let i = 0; i < this.collisionArray.length; i++) {
        const instance = this.collisionArray[i];
        if (instance.type === "wall" || !instance.damage) {
          continue;
        }
    ...
    if (this.isDead() && !this.readyToDie) {
      ...
      const jackpot = util.getJackpot(this.skill.score) / this.collisionArray.length;
      ...
      const killCounts = {};
      for (const { label } of killTools) {
        if (!killCounts[label]) {
          killCounts[label] = 0;
        }
        killCounts[label]++;
      }
      const killCountEntries = Object.entries(killCounts).map(([name, count], i) => name);
  ...
  destroy() {
    ...
    // Remove from minimap
    const i = global.gameManager.minimap.findIndex(entry => {
      return entry[0] === this.id;
    });
    if (i != -1) {
      util.remove(global.gameManager.minimap, i);
    }
