class Tag {
    ...
    Config.tag_data = {
      ...
      addPlayer: entity => {
        ...
        entity.on("dead", ({ killers }) => {
          ...
          const players = killers.filter(entity => entity.isPlayer || entity.isBot);
          if (players.length) {
      ...
      addBot: entity => {
        ...
        entity.on("dead", ({ killers }) => {
          Config.tag_data.removeFromTeam(team);
          const killer = killers.filter(entity => entity.isPlayer || entity.isBot);
          if (killer.length) {
            global.nextTagBotTeam = killer[0].team;
          }
        });
      }
    };
  ...
  checkWin() {
    if (this.won || !this.canStart || !global.gameManager.clients.length) {
      return;
    }
    let bestTeam = -1;
    ...
    if (!this.teams.every((team, i) => i === bestTeam || team === 0) || this.teams[bestTeam] < 5) {
      return;
    }
    this.won = true;
