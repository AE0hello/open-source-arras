const calculatePoints = wave => 5 + wave * 3;
...
class bossRush {
  ...
  generateWaves() {
    ...
    for (let i = 0; i < this.length; i++) {
      ...
      while (points > 0 && choices.length) {
        ...
        const [cost, boss] = ran.choose(choices);
        points -= cost;
        wave.push(boss);
      }
    ...
    o.on("dead", () => {
      if (o.team === TEAM_ENEMIES) {
        ...
        if (this.leftSanctuaries == 0) {
          global.gameManager.socketManager.broadcast("You can now respawn.");
        }
        ...
        if (this.gameActive) {
          util.remove(this.room.spawnable[TEAM_BLUE], this.room.spawnable[TEAM_BLUE].indexOf(tile));
        }
        util.remove(this.sanctuaries, this.sanctuaries.indexOf(o));
        const newTeam = TEAM_ENEMIES;
        ...
        if (this.leftSanctuaries == 0) {
          ...
          const loop = setInterval(() => {
            if (this.leftSanctuaries !== 0) {
              global.cannotRespawn = false, clearInterval(loop);
            }
            timeRemaining--;
            if (timeRemaining == 0) {
              this.playerLose();
  ...
  spawnEnemyWrapper(loc, type) {
    ...
    if (Config.fortress || Config.citadel) {
      enemy.controllers.push(new ioTypes.bossRushAI(enemy, {}, global.gameManager));
    }
  ...
  spawnWave(waveId) {
    ...
    // spawn bosses
    for (const boss of this.waves[waveId]) {
      const spot = ran.choose(global.gameManager.room.spawnable.bossSpawnTile).randomInside();

      const enemy = this.spawnEnemyWrapper(spot, boss);
    ...
    const newSancTier = Math.min(Math.floor(this.waveId / 5) + 1, 6);
  ...
  start(mazeType) {
    ...
    for (let i = 0; i < Class.director.UPGRADES_TIER_2.length; i++) {
      const string = Class.director.UPGRADES_TIER_2[i];
      if (string === "underseer") {
        Class.director.UPGRADES_TIER_2.splice(i, 1);
        Class.menu_unused.UPGRADES_TIER_0.push("underseer");
      }
    }
    for (const tile of this.room.spawnable[TEAM_BLUE]) {
    ...
    if (mazeType) {
      const mazeGenerator = new global.mazeGenerator.MazeGenerator(mazeType);
      const { squares, width, height } = mazeGenerator.placeMinimal();
      squares.forEach(element => {
        ...
        if (Config.spooky_theme) {
          const eyeSize = 12 * (Math.random() + 0.45);
          const spookyEye = new Entity({ x: wall.x + (wall.size - eyeSize * 2) * Math.random() - wall.size / 2, y: wall.y + (wall.size - eyeSize * 2) * Math.random() - wall.size / 2 });
