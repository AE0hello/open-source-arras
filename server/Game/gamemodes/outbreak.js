class Outbreak {
  ...
  zombify(liveEntity) {
    ...
    if (liveEntity.defs[0] == "bot") {
      liveEntity.defs = liveEntity.defs[1];
    }
    const zombieEntity = new Entity({ x: liveEntity.x, y: liveEntity.y });
    ...
    setTimeout(() => {
      const Class = ensureIsClass(liveEntity.defs[0]);
