...
const commands = [
    ...
    run: ({ socket, level }) => {
      ...
      const lines = [
        ...
        ...commands.filter((c) => level >= c.level && !c.hidden).map((c) => {
          ...
          const commandText = cmdData.map((e) => e.map((name) => name).join(` or ${prefix} `)).join(" ");
          const description = c.description ?? false;
  ...
  },
  {
    ...
    level: 0,
    run: ({ socket, args }) => {
      const sendAvailableLeaderboardMessage = () => {
        const lines = [
  ...
  },
  {
    ...
    run: ({ socket, args, gameManager }) => {
      const sendAvailableArenaMessage = () => {
        const lines = [
          "Help menu:",
      ...
      if (!args[0]) {
        sendAvailableArenaMessage();
      } else {
        switch (args[0]) {
          case "size":
            if (args[1] === "dynamic") {
              if (!Config.sandbox) {
                return socket.talk("m", 3_000, "This command is only available on sandbox.");
              }
          ...
          case "team":
            if (!args[1]) {
              return socket.talk("m", 3_000, "Invalid argument.");
            }
            if (args[1] === "0") {
              Config.mode = "ffa";
              Config.teams = null;
  ...
  },
  {
    ...
    run: ({ socket, args, gameManager }) => {
      ...
      if (command === "reloaddefs" || command === "redefs") {
        ...
        const sinceLastReload = time - global.reloadDefinitionsInfo.lastReloadTime;
        ...
        for (const entity of entities.values()) {
          ...
          if (!["tank", "miniboss", "food"].includes(entity.type)) {
            continue;
          }
          if (entity.bond) {
            continue;
          ...
          const entityDefs = JSON.parse(JSON.stringify(entity.defs));
          // Save color to put it back later
          const entityColor = entity.color.compiled;
...
function runCommand(socket, message, gameManager) {
  if (!message.startsWith(prefix) || !socket?.player?.body) {
    return;
  }

  const args = message.slice(prefix.length).split(" ");
  const commandName = args.shift();
  const command = commands.find((command) => command.command.includes(commandName));
  if (command) {
    const permissionsLevel = socket.permissions?.level ?? 0;
