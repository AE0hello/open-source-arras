const { MockupEntity } = require("./mockupEntity");
...
function checkIfOnLine(endpoint1, endpoint2, checkPoint) {
  const xDiff = endpoint2[0] - endpoint1[0],
...
function getDimensions(entity) {
  ...
  const { x, y, r } = constructCircumcirle(point1, point2, point3);
...
function constructCircumcirle(point1, point2, point3) {
  ...
  const x1 = util.rounder(point1[0]);
  const y1 = util.rounder(point1[1]);
  const x2 = util.rounder(point2[0]);
  const y2 = util.rounder(point2[1]);
  let x3 = util.rounder(point3[0]);
  const y3 = util.rounder(point3[1]);
...
function sizeEntity(entity, x = 0, y = 0, angle = 0, scale = 1) {
  ...
  if (entity.shape < 3 || entity.shape >= sidesMax) {
    for (let i = 0; i < sidesMax; i++) {
      const theta = Math.PI * 2 / sidesMax * i;
      endPoints.push([x + Math.cos(theta) * scale, y + Math.sin(theta) * scale]);
    }
  } else {
    // Process body as true size and shape otherwise
    const angleOffset = (entity.shape % 1) * 2 * Math.PI;
    const numSides = Math.floor(entity.shape);
  ...
  for (const g of entity.guns) {
    ...
    const widths = g.aspect > 0 ? [g.width * g.aspect / 2, g.width / 2] : [g.width / 2, -g.width * g.aspect / 2],
      points = [],
  ...
  const turretsAndProps = entity.turrets.concat(entity.props);
  for (const t of turretsAndProps) {
    const trueAngle = angle + t.bound.angle,
...
function buildMockup(className, Manager) {
  try {
    ...
    const mockup = new MockupEntity(Manager);
    ...
    for (const i in Class[className]) {
      util.error("\t" + i + ": " + Class[className][i]);
    }
    throw error;
  }
}
