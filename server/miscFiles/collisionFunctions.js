...
function firmcollidehard(my, n, buffer = 0) {
  ...
  if (buffer > 0 && dist <= my.realSize + n.realSize + buffer) {
    const repel = (my.acceleration + n.acceleration) * (my.realSize + n.realSize + buffer - dist) / buffer / Config.runSpeed;
...
function advancedcollide(my, n, doDamage, doInelastic, nIsFirmCollide = false) {
  ...
  // radius check
  if (component < difference.length - combinedRadius) {
    return;
  }
  ...
  if (!deltaLengthSquared || det < 0 || C < 0) { // This shall catch mathematical errors
    ...
    const t1 = (-B - Math.sqrt(det)) / (2 * deltaLengthSquared),
  ...
  if (doDamage) {
    const speedFactor = { // Avoid NaNs and infinities
    ...
    if (!bail && !my.invuln && !n.invuln) {
      ...
      const resistDiff = my.health.resist - n.health.resist,
        damage = {
  ...
  if (n.healer && n.team == my.team && n.master.id != my.id) {
    return;
  }
  if (my.healer && n.team == my.team && my.master.id != n.id) {
    return;
  }
  /** *********** DO MOTION ***********/
  if (nIsFirmCollide < 0) {
    ...
    if (my.knockback && n.knockback) {
      ...
      knockback = Config.knockback_multiplier;
    }
    ...
    const spring = 2 * Math.sqrt(savedHealthRatio._me * savedHealthRatio._n) / Config.run_speed,
      elasticImpulse =
...
function mooncollide(moon, bounce) {
  const collisionRadius = util.getDistance(moon, bounce);
  ...
  const angleFromMoonToBounce = Math.atan2(bounce.y - moon.y, bounce.x - moon.x);
  ...
  const tangentVelocity = bounce.velocity.length * Math.sin(angleFromMoonToBounce - velocityDirection);
  const perpendicularVelocity = bounce.velocity.length * Math.cos(angleFromMoonToBounce - velocityDirection) * elasticity * -1;
  ...
  const newVelocityMagnitude = Math.sqrt(tangentVelocity ** 2 + perpendicularVelocity ** 2);
...
function mazewallcollide(wall, bounce) {
  if (bounce.god === true || bounce.passive === true || bounce.isArenaCloser || bounce.master.isArenaCloser) {
    return;
  }
  if (bounce.store.noWallCollision) {
    return;
  }
  ...
  for (let i = 0; i < 4; i++) {
    if (!collisionFaces[i] | extendedOverFaces[(i + 3) % 4] | extendedOverFaces[(i + 1) % 4]) {
      continue;
    }
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (Math.hypot(bounce.x - cornerX, bounce.y - cornerY) > bounce.size) {
      return;
    }
    mazewallcollidekill(bounce, wall);
...
function mazewallcustomcollide(wall, bounce) {
  /* By LA3T */
  if (!bounce || bounce.ac || bounce.passive) {
    return;
  }
