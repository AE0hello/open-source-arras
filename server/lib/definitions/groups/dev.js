...
// Greater Arena Closer Tank
Class.greaterArenaCloser = {
  PARENT: "genericTank",
  LABEL: "Greater Arena Closer",
  NAME: "Greater Arena Closer",
  DISPLAY_NAME: false,
  COLOR: "yellow",
  ...
  GUNS: [{
    ...
    PROPERTIES: {
      SHOOT_SETTINGS: combineStats([g.basic, g.arenaCloser, { reload: 0.1, recoil: 0, damage: 1e100 }]),
      TYPE: ["bullet", { LAYER: 12 }]
    }
  }]
};

// Ciper Probe's Stuff Menu
Class.ciperProbesStuff = makeMenu("Ciper Probe's Stuff");
Class.ciperProbesStuff.UPGRADES_TIER_0 = [
  "opTanks",
  "mapEntities"
];
...
Class.gamemodeArenaCloser = {
  PARENT: "genericTank",
  LABEL: "Arena Closer",
  NAME: "Arena Closer",
  UPGRADE_LABEL: "Gamemode A.C.",
  DISPLAY_NAME: false,
  DANGER: 8,
  SIZE: 34,
  COLOR: "yellow",
  ...
  VALUE: 50000,
  BODY: {
    REGEN: 1e4, // Reduced from 1e5
    HEALTH: 1e5, // Reduced from 1e6
    DENSITY: 30,
    DAMAGE: 1e4, // Reduced from 1e5
    FOV: 10,
  ...
  GUNS: [{
    POSITION: [14, 10, 1, 0, 0, 0, 0],
    PROPERTIES: {
      SHOOT_SETTINGS: combineStats([g.basic, g.arenaCloser, { reload: 1.0, damage: 0.1, speed: 0.5 }]),
      TYPE: "bullet"
    }
  }]
};

// Map Entities Submenu
Class.mapEntities = makeMenu("Map Entities");
Class.mapEntities.PROPS = [
  {
...
Class.mapEntities.UPGRADES_TIER_0 = [
  "gamemodeArenaCloser"
];
...
Class.menu_addons.UPGRADE_TOOLTIP = "Content that is (usually) not part of Arras but was added by someone else.";
...
Class.menu_testing.UPGRADE_TOOLTIP = "A large selection of tanks that use many of the features of Arras. \n" +
  ...
  ON: [
    {
      ...
      handler: ({ body, globalMasterStore: store, gun }) => {
        if (gun.identifier !== "switcherooGun") {
          return;
        }
...
const testLayeredBoss = new LayeredBoss("testLayeredBoss", "Test Layered Boss", "terrestrial", 7, 3, "terrestrialTrapTurret", 5, 7, true, { SPEED: 10 });
