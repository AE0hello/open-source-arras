...
function mergeDreads(dread1, dread2, sourceDread, tier) {
  const dread1Name = dread1;
  const dread2Name = dread2;
  ...
  // Label it
  if (dread1.LABEL == dread2.LABEL) {
    LABEL = dread1.LABEL;
  }
  if (dread1.UPGRADE_TOOLTIP == dread2.UPGRADE_TOOLTIP) {
    UPGRADE_TOOLTIP = dread1.UPGRADE_TOOLTIP;
  }

  // Guns
  if (dread1.GUNS) {
    GUNS.push(...dread1.GUNS);
  }
  for (const g in dread2.GUNS) {
    const POSITION = JSON.parse(JSON.stringify(dread2.GUNS[g].POSITION)),
  ...
  // Turrets
  if (dread1.TURRETS) {
    TURRETS.push(...dread1.TURRETS);
  }
  if (dread2.TURRETS) {
    TURRETS.push(...dread2.TURRETS);
  }

  // Body
  if (dread1.BODY) {
    for (const m in dread1.BODY) {
      BODY[m] += (dread1.BODY[m] - dreadnoughtBody[m]) / 2;
    }
  }
  if (dread2.BODY) {
    for (const m in dread2.BODY) {
      BODY[m] += (dread2.BODY[m] - dreadnoughtBody[m]) / 2;
    }
  }

  // Definition name
  const definitionName = dread1Name + dread2Name;
  ...
  const upgradeLevel = `UPGRADES_TIER_${eval(`tier${tier}`)}`;
  util.forcePush(Class[sourceDread], upgradeLevel, definitionName);

  // Generate new dreads recursively
  if (!dread1.UPGRADES_TIER_M1 || !dread2.UPGRADES_TIER_M1) {
    return;
  }

  for (const upgrade1 of dread1.UPGRADES_TIER_M1) {
    for (const upgrade2 of dread2.UPGRADES_TIER_M1) {
...
for (const branch1 of Class.dreadOfficialV1[`UPGRADES_TIER_${tier1}`]) {
  for (const branch2 of t1Bodies) {
